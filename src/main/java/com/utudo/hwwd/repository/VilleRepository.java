...
public interface VilleRepository extends JpaRepository<Ville,Long> {
    ...
    @Override
    Ville getOne(Long aLong);

    @Override
    Optional<Ville> findById(Long aLong);

    @Override
    List<Ville> findAllById(Iterable<Long> iterable);

    @Override
    void deleteById(Long aLong);


    @Query(nativeQuery = true,value = "select * from hw_ville where postcode like %:postcode% limit 10")
    List<Ville> findVilleByPostcodeParamsNative(@Param("postcode") String postcode);



    @Query(nativeQuery = true,value = "select * from hw_ville where postcode like %:keyword% or `name` like %:keyword% limit 10")
    List<Ville> findVilleByKeywordsParamsNative(@Param("keyword") String keyword);
