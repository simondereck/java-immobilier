$(function () {
    ...
    function setMap(){
        ...
        map = new mapboxgl.Map({
            ...
            // center: [3.4743641, 46.5171466], // starting position [lng, lat]
            center:[lng,lat],
    ...
    function setLocation(lat,lng) {
        ...
        $.post("/site/map/annonces",params,function (data) {
            if (data.status===1){
                dataArray = data["latlngs"];
                dataVille = data["villes"];
                $("#annonces").html("");
                if (markers.length>0){
                ...
                for (let i = 0; i < data["latlngs"].length; i++) {
                    ...
                    if (latlng.annonce){
                        ...
                        if (annonce.location && dataVille[annonce.location]){
                            annonceDiv += "<div class='title'> " + dataVille[annonce.location].name +" </div>";
                        }

                        if (annonce.furniture){
                            annonceDiv += "<div class='title'>家具齐全 " + annonce.size +" 平</div>";
                        }else{
                            annonceDiv += "<div class='title'>不带家具 "+ annonce.size +" 平</div>";
                        }

                        annonceDiv += "<div class='price'>"+annonce.price+" EURO</div>" +
                            "<a href='/location/house/detail/"+annonce.id+".html'>详情</a>" +
                            "</div></div>";

                        $("#annonces").append(annonceDiv).show("toggle");
    ...
    function addLatLng(latlng){
        ...
        marker.getElement().addEventListener('click',function () {
            ...
            if (annonce.location && dataVille[annonce.location]){
                popWindow += "<div class='title'> " + dataVille[annonce.location].name +" </div>";
            }else{
                // popWindow += "<div class='title'>"+villes[vkey].name+"</div>"

            }


            if (annonce.furniture){
                popWindow += "<div class='title'>家具齐全 " + annonce.size +" 平</div>";
            }else{
                popWindow += "<div class='title'>不带家具 "+ annonce.size +" 平</div>";
            }
    ...
    $('select[name=rtype]').on('change', function() {
        initPage();
    });
    ...
    function initPage(url) {
        let urlPath = url ? url : "/site/map/search";
        let dataParams = $("#searchAnnonces").serializeArray();
        let params = {};
        if (isMobilePhone){
            if ($("#search-div") && $("#search-div").is(":visible")){
                $("html").remove("search-icon-show");
                $("#search-div").hide();
            }
        }
            ...
            if (data.status===1){
                ...
                for (let i = 0; i < data["latlngs"].length; i++) {
                    ...
                    if (latlng.annonce){
                        ...
                        if (annonce.location && vkey){
                            annonceDiv += "<div class='title'>"+villes[vkey].name+"</div>"
                        }else{
                        ...
                        annonceDiv += "<div class='price'><h4>"+annonce.price+" EURO</h4></div>" +
                            "<a href='/location/house/detail/"+annonce.id+".html'>详情</a>" +
                            "</div></div>";
